...
```
The  mean total number of step per day is `r as.character(round(avg,2))`, where the median value is `r as.character(round(med,2))`.
...
```
Since the dataset contains 5-minutes interval a total of `r 24*12` interval data has been generated (`t.interval.steps`). It may be noted that there is a peak at `r peak_hour` o'clock.
