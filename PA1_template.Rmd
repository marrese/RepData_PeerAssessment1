...
```
The rounded mean total number of step per day is `r round(avg,0)`, where the median value is `r round(med,0)`.
...
```
Since the dataset contains 5-minutes interval a total of `r 24*12` interval data has been generated (`t.interval.steps`). May be noted that there is a peak at `r peak_hour` o'clock.
